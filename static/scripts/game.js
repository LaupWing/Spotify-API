...
function createGameEnviroment(users){
    ...
    removeElements(body)
    const newElement = '<main class="container"><div id="media"></div><div class="track_guess"><h2>Track starts in</h2></div><div class="track_reveal-container"><img></img><div class="track-reveal"><h2 class="artist_name"></h2><p class="song_name"></p></div></div></main><form id="player_answer"><div class="answer-container"><div class="answer"><h2>Artist</h2><input class="artist_input" type="text"></div><p>-</p><div class="answer"><h2>Song</h2><input class="song_input" type="text"></div></div><button>confirm</button></form><ul id="users"></ul>'
...
function setTrack(track){
    ...
    const readyMsg = '<div class="readyMsg"></div>'
    document.querySelector('main .track_guess').insertAdjacentHTML('beforeend', readyMsg)
    setUpSVG()
    startTimer()
}
...
function startTrack(){
    ...
    document.querySelector('main .track_guess svg #Playbutton').classList.remove('transparent')
